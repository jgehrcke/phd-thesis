...
with a minimum of \SI{8}{\angstrom} distance between solute and box boundaries. The
systems were neutralized by adding Na$^{+}$ or Cl$^{-}$ counterions. The MD time
integration step was set to \SI{2}{\femto\second}, whereas bonds including hydrogen
...
\SI{8}{\angstrom}. The Particle Mesh Ewald method was used for treating long-range
...
\SI{20}{\pico\second} in the canonical ensemble (NVT) using the Langevin thermostate
and periodic boundary conditions. Subsequently, \SI{500}{\pico\second} of MD in the
...
grid step of \SI{0.38}{\angstrom}. Grid size, position and orientation were chosen
...
and carboxyl groups to be flexible during the search. \num{e3} independent docking
runs were performed for each complex. Each docking run was performed by a
genetic algorithm (initial population size: 300, abortion condition: \num{e5}
...
For single test cases with $T=\SI{4}{\nano\second}$ we observed a quickly fluctuating
$RMSd_{\mathrm{rot,trans,int}}(t)$ curve with ligand displacements from the
shortest path between 5 and \SI{40}{\angstrom} $RMSd$. In contrast, when using
$T=\SI{0.5}{\nano\second}$, the displacement varied in the range between 0 and
\SI{10}{\angstrom} $RMSd$. From these data, we could already conclude that when
using $T=\SI{4}{\nano\second}$, the limiting factor for translational and rotational
...
ligand during tMD with $s=\SI{30}{\angstrom}$ and $T=\SI{4}{\nano\second}$ in detail, we
...
\SI{20}{\angstrom}. The path variation among ligand trajectories (the standard
deviation of the averaged data) is about $\pm\SI{10}{\angstrom}$ $RMSd$. The data
suggest that with $s=\SI{30}{\angstrom}$ and $T=\SI{4}{\nano\second}$ enough translational
...
$t=\SI{2}{\nano\second}$ (\cref{fig:dmd:sampling}), which is probably caused by the
...
$T=\SI{10}{\nano\second}$. This allows for significant ligand refinement according to
...
\SI{1.0 +- 0.4}{\angstrom} (mean value and standard deviation derived from all TDS
