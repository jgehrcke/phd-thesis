...
seek to extract as many ensemble properties of a DMD study as possible.
\textit{Static} coordinate-based docking solution ensemble evaluation by means
of clustering has been discussed before in \cref{chapter:clustering}. Here, we
describe an additional approach for extracting \textit{dynamic} DMD study
ensemble properties that are likely to provide essential insights about the
...
receptor that are most important for binding, whereas the entire DMD study
provides the corresponding data, rendering the outcome quite significant and
...
for single residues in the receptor. The filtering step makes sure that no
invalid (e.g.\ unbound) docking solutions contribute to the final result. The
...
protein residues. Simply spoken, such data merging helps identifying those
