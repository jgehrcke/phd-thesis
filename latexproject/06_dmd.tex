...
This chapter is a customized version of the publication \enquote{Flexibility and
...
\cite{dmd_samsonov_gehrcke_2014}. It describes a novel method, Dynamic Molecular
Docking (DMD), which is a targeted molecular dynamics-based protocol developed
to address ligand and receptor flexibility as well as the inclusion of explicit
solvent in local molecular docking. The goal of this work has been to develop a
proof of concept for an MD-based docking approach and to analyze its
applicability for protein-GAG systems, for which it is conceptually well-suited:
DMD exploits the electrostatics-driven attraction of a ligand to its receptor,
treats both as entirely flexible and considers solvent explicitly.

In the validation study presented here, DMD was shown to have high predictive
significance for systems dominated by electrostatic attraction and demonstrated
its capability to reliably identify the receptor residues contributing most to
binding.

Sergey Samsonov (SS) and Jan-Philip Gehrcke (JPG) equally contributed to the
method development and validation study design. The figures have been prepared
by JPG.


., performed the analysis, and wrote the manuscript. ABC and GHI collected the data.

The overall method development and study design was conducted by  in equal terms.



A class of ligands for which docking performance    where we have
published the description and validation of Dynamic Molecular Docking.
