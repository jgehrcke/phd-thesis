...
parameters and a finite element grid spacing of \SI{1}{\angstrom}. The atomic
radii and point charges of the protein were parameterized according to the
FF99SB force field \cite{case_amber_12}. After contributing Fortran software
patches back to the AmberTools project, we were able to extract the Coulomb data
from PBSA in units of \si{\kilo\calory\per\mole\per\elementarycharge} and in a
file format readable by the visualization software VMD \cite{vmd_1996}.

\subsection{Coulomb potential evaluation}

For authors discussing the electrostatic properties of a certain receptor
protein it is not uncommon to show the electrostatic potential mapped onto the
molecular surface, and to draw conclusions solely based on this view, as done
for instance in

 base the electrostastic potential


estatic_proteins_warshel_2006

quote from there that at boundary, ..


Calculations of the electrostatic potential of molecules are most error-prone
near the dielectric boundary, i.e.\ on the molecular surface. We therefore did
not simply map the potential on the molecular surface but analyzed the topology
of the potential with an isosurface representation while varying the isosurface
value. This allows for an understanding  of the distribution of the potential in
space and how strongly it would affect a ligand. \cref{fig:bspred:sdf1_estatic}
shows an isosurface of the Coulomb potential of SDF-1.
\cref{fig:bspred:various_estatic} shows analogous isosurface representations of
the electrostatic potential for the other TDS complexes. For this type of
