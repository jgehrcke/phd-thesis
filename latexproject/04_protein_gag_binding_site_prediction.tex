...
We can conclude that the presented method is very well able to make valid
predictions regarding protein-GAG binding regions.
...
The most important aspect to discuss when predicting certain behavior is the
certainty of the prediction. A prediction is only a useful when we can estimate
its certainty.  Compared to
docking methods, this method is significantly less complex: it is a bare-bones
method, incorporating a quite simple physical model, whereas docking methods
usually apply a combination of physical as well as phenomenological model. The
certainty of a docking prediction is difficult to assess if the docking method
is applied outside its scope of validity, i.e.\ to different systems and search
space sizes than it has been developed for. Strictly spoken, the certainty is
not assessable, neither quantitatively, nor qualitatively. The reason for this
is the large complexity of a docking method, as of which we can not
systematically describe how the method should behave, if we leave the scope of
validity.


Here, we see a clear advantage of the Coulomb potential evaluation over more
complex methods such as global docking.


Analysis of the CD44-HA system is a good example for making this point, since based on the electrostatic potential topology we
would not have

makes obvious that

In our opinion, there are two major conclusions to be drawn from this study:


- Analysis of the electrostatic potential of a protein can not be used
- it can not tell whether it binds gags or not
- distinct region of significant electrostatic attraction, that this is the place where GAGs would most likely bind
-

The described method of electrostatic potential evaluation lets us draw c
...
\textit{a priori} exclude regions of the receptor surface as a potential GAG
binding region, when repulsive to negatively charged ligands.
...
determinant.
 ...
 -- that is obvious from what is known about protein-GAG systems so
far.
...
Conceptual analysis of protein-GAG interaction in general
