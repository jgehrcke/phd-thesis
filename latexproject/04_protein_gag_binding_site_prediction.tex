...
above would have led us to state that \enquote{we can not tell where a GAG would
bind, however it will most likely not bind in the repulsive regions}. At the
same time the data regarding FGF2 and SDF-2 provide undeniable support that GAGs
bind where predicted, especially regarding the large characteristic
electrostatic potential isovalues in proximity of the molecular surface. The
main conclusion is that if we observe a \textit{distinct} region of
\textit{significant} electrostatic attraction, then this is the place where GAGs
would most likely bind. If a protein is known to bind GAGs, and the
...
According to recent literature in the field (see motivation part above), global
docking methods are often (mis)used for protein-GAG binding site prediction,
because no other tool seems to be established. With the presented method we seek
to provide such a tool to the field. Hence, the presented method is in direct
competition with (global) docking methods. Compared to those, Coulomb potential
evaluation is significantly less complex: it is a bare-bones method based on a
simple physical model, whereas docking methods usually apply a combination of a
more complex physical as well as a phenomenological models. The certainty of a
docking prediction is difficult to assess if the docking method is applied
outside of its scope of validity, i.e.\ to different systems and e.g.\ search
space sizes than it has been optimized for. A reason for this is the complexity
of a docking method, as of which we can not systematically understand the
behavior of the method if we leave the scope of validity. Strictly spoken, in
that case the certainty is \textit{not} assessable, neither quantitatively, nor
qualitatively. Therefore, the risk for producing false-positive predictions is
much higher than in case of the method presented by us. Here, we see a clear
conceptual advantage of the Coulomb potential evaluation over more complex
methods such as global docking.
...
possible via mapping of the potential to the molecular surface only. For
instance, this can be helpful for identifying repulsive regions in space to
\textit{a priori} exclude large regions of the protein surface as potential
...
However, we have also learned that electrostatic interaction may

We have learned from this analysis that Coulomb interaction can be
the primary determinant for the GAG binding site and if it is, it becomes
obvious.
...
Besides qualitative statements about a binding region, the method presented here
might also provide insights about the binding \textit{affinity} of a certain
protein-GAG system, via evaluation of the the characteristic isovalue. However,
the method can in all generality not be used for predicting whether a certain
binding mode would be \textit{specific} or not.
...
method is that we can say: *if* a GAG binds to that protein, it most likely
