...
Another advantage of the visualization of the electrostatic potential is that it
enables the researcher to quickly grasp the global electrostatic properties of
any given protein. For instance, it can be helpful to \textit{a priori} exclude
large regions of the protein surface as potential GAG binding region, when
repulsive to negatively charged ligands.
...
\textit{affinity} via the characteristic isovalue, however
it also can not be used for predicting whether a certain binding mode would be
\textit{specific} or not.
