...
electrostatic potential of FGF2 with an isosurface representation for varying
isovalues. The four isovalues shown are positive ones, and the surfaces
therefore represent \textit{attraction} for GAGs. If we had shown isosurfaces
for isovalues of comparable absolute value and opposite sign, those would be
entirely hidden \enquote{within} the molecular surface, i.e.\ electrostatic GAG
repulsion is negligible in case of FGF2.

\Cref{fig:bspred:fgf2_multi_iso}a shows the isosurface corresponding to the
smallest of the four isovalues
$\Phi=\Si{2.2}{\kilo\calory\per\mole\per\elementarycharge}$, whereas this is
still a strong potential, yielding significant potential energies for charged
ligand molecules: \Si{2.2}{\kilo\calory\per\mole} is almost four times larger
than the thermal energy at \Si{300}{\kelvin}. From this representation we learn
that FGF2 is highly polar on a global scale: in the top direction, the
isosurface protrudes into space far beyond the molecular surface, whereas it
does not surpass the molecular surface at the bottom. We conclude from this
representation that FGF2 may interact with GAGs over quite large distances
(several times larger than what can be considererd a molecular contact), with a
clear preference towards one side of FGFs. We observe that the experimentally
determined natural heparin ligand pose is bound on that side of the protein.

\Cref{fig:bspred:fgf2_multi_iso}b and c visualize the process of increasing
the isovalue for the isosurface shown. This process serves two purposes:
\textit{ii)} isosurface shape changes with increasing isovalue
provide insights about the direction into which a negatively charged
molecule is \textit{guided} by the electrostatic potential once caught.

 \textit{1)} for further narrowing down those regions in space near the molecular surface of FGF2 where electrostatic attraction is
strongest. and
