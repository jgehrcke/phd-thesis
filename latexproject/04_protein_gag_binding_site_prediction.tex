...
success for protein-GAG binding region prediction. Therefore these studies lack
the capability of further enlightening the theoretical basis of protein-GAG
interaction, since a good nonarbitrary explanation would be a step towards
...
other contributions for larger distances. As of these considerations, we
...
indicating that classical molecular docking is an overkill approach for binding
region prediction. Here, we see a clear conceptual advantage of the Coulomb
...
prediction for any given system falsifies the docking approach as a whole, which
is a conjunction of many statements (scoring and search are comprised of various
subcomponents). Failure shows that one or more of these statements is false, but
it does not show which one \cite{savage1990scientific}. Falsification of our
\textit{simplistic} hypothesis on the other hand would more easily translate
into explanatory force and gain of scientific knowledge. So far, our hypothesis
is corroborated by the reference systems discussed above as well as other
systems we have looked at, i.e.\ it is a hypothesis that has passed certain
tests --- however, it has not been proven right (generally, scientific theories
can not be proved, only disproved \cite{schafersman_scientific_method}).
...
proving the prediction right must be considered luck, proving the prediction
wrong would not provide fundamental insights about why the applied model was
insufficient to describe the system.
