...
the type of molecules investigated, there are various sources of error that can
...
\subsection{Methods} G. Künze performed NOESY experiments for
measuring heparin-internal hydrogen-hydrogen (${}^1$H-${}^1$H) distances for
heparin tetrasaccharide (HP dp4) molecules in solution, in two different
conditions: once under presence of \hl{(murine)} IL-10 in the very same
solution, and once without presence of IL-10. The NMR protocol details of these
experiments have been published in \cite{kuenze_gehrcke_2014}. In principle, the
geometry information encoded in the obtained NOESY data can be used for creating
two HP dp4 structure models, one for what we call the \enquote{bound} state, and
one for the \enquote{unbound} state (with and without IL-10, respectively).
...
the experimentally obtained distance constraints as good as possible. I have
...
with a known distance of \SI{2.55}{\angstrom} in the ${}^4$C${}_1$ chair
conformation. A tolerance interval of $\pm \SI{0.3}{\angstrom}$, $\pm
\SI{0.6}{\angstrom}$ and $\pm \SI{1.0}{\angstrom}$ was used for measured
distances $< \SI{2.0}{\angstrom}$, between \num{2.0} and $\SI{4.0}{\angstrom}$,
and $> \SI{4.0}{\angstrom}$, respectively.
...
referred to as rings A, B, C, D, respectively (the GAGs used for the experiments
were prepared by enzymatic digestion, so the non-reducing terminal uronic acid
became unsaturated and is therefore labeled $\Delta$UA,2 here). Sulfate partial
charges were obtained from RESP calculations for methylsulfate at the 6-31(d)G
level of theory. The first structure optimization stage was a simulated
annealing protocol using a simple implicit solvent model with a 1/r distance
dependency of the dielectric constant. The protocol contained the following
