...
\subsection{Background: NOESY forinter-atomic distance determination}

In essence, the spins of atomic nuclei that are spatially close to each other
(closer than about \SI{5}{\angstrom}) undergo a significant dipole-dipole
...
Overhauser effect spectroscopy (NOESY) experiment, this correlation effect
manifests itself in form of off-diagonal resonance peaks. The intensity of such
a so-called cross peak is proportional to the cross-relaxation rate constant for
the spin system. Due to the dipolar nature of the spin-spin interaction the rate
constant is proportional to the inverse sixth power of the distance between the
two spins \cite{neuhaus2000_noe}.

epends inversely on the distance, hence
the restrain

 via cross-relaxation


Nuclear Overhauser cross relaxation

wikipedia

In NMR spectroscopy, the Solomon equations describe the dipolar relaxation process of a system consisting of two spins.

 for transferring magnetization from one spin to the other

 between nuclei which are physically close to each other regardless of whether there is a bond between them

 the NOE occurs through space, not through chemical bonds.
...
transfer of nuclear spin polarization from one nuclear spin population to another via cross-relaxation
...
spin–lattice relaxation.
 ...
 once with and once without presence of
\hl{murine} IL-10. The geometry information in this data can be used for
creating HP dp4 structure models for what we call the \enquote{bound} and
\enquote{unbound} state. Creating structure models from this data is an
...
Our collaborator G. Künze performed nuclear Overhauser effect spectroscopy
(NOESY) for measuring heparin-internal hydrogen-hydrogen distances for a
heparin tetrasaccharide (HP dp4) in solution.
