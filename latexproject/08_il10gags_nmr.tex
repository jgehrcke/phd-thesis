...
in solution, in two different conditions: under presence of \hl{(murine)} IL-10
in the very same solution, and without presence of IL-10. The protocol details
of these experiments have been published in \cite{kuenze_gehrcke_2014}. The aim
of these experiments was to extract and use the geometry information encoded in
the resulting NOESY data, for creating two HP dp4 structure models: one for what
...
The orientation of the 6-O-sulfates of the rings B and D, however, was observed
to be affected by the presence of IL-10 (see
\cref{fig:nmr:hp_sulfate_orientations}). The 6-O-sulfate of ring B showed a
slight shift in the rotamer distribution between unbound and bound state,
whereas in the bound state there is a \SI{10}{\degree} deviation from the ideal
rotation angle of \SI{-60}{\degree}. The 6-O-sulfate of ring D only populated a
small fraction of the \textit{gauche-trans} conformer (\SI{60}{\degree}) in the
presence of IL-10. The meaning of these observations is further discussed in
...
Discussion: usually, ... requires full relaxation matrix calculations
(\cite{foster_mulloy_1993,mikhailov_hp_tetra_1996}), which also requires more
data to be obtained from the NMR experiments... nevertheless, the analysis as we
performed it usually has enough accuracy for obtaining useful results bla bla
\cite{jones_noe_2011}.
...
Furthermore, according to our heparin structure models,
