...
Molecular \enquote{docking} is a method for predicting the stable binding mode
of a given receptor-ligand system. That is, molecular docking aims to solve an
optimization problem, in which the optimal binding configuration between two
molecules is searched for. Hence, the two main ingredients required for any
...
Molecular docking methods are usually developed with the goal to be
computationally efficient, i.e.\ a number of severe approximations is usually
implemented in docking methods. Among others, the following approximations are
common among classical docking methods:
...
statically, i.e.\ it has zero degrees of freedom.
\item Flexibility in the ligand molecule is often only approximated: bond
lenghts are fixed, and certain torsion angles are also kept fixed.
\end{itemize}
...
In previous studies, AutoDock 3 (AD3) \cite{Morris1998} has been shown to be
able to produce reasonable docking results for protein-GAG systems
