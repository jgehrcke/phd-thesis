...
obtained for the bound state, i.e.\ for the receptor-ligand complex (then called
...
these numbers, a free energy of binding estimate, often called $\Delta G$, can
be derived:
...
literature, this is usually listed, $E$ is called \textit{free energy} $G$, and
then it is argued that the entropy term can and should be neglected when
comparing similar molecular systems: the methods for estimating the loss of
entropy upon binding based on MD simulations are rather weak, and can be assumed
to be similar for comparable binding modes, so often the best option for now is
to neglect that term \cite{ryde_mmpbsa_2006, homeyer_gohlke_2012}.

homeyer_gohlke_2012

 of
 called
the free energy G of the



system, with an additional entropy , with  entropy neglected
