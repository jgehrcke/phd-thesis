...
PDB with identification code 2ILK. This structure (PDB 2ILK) was used throughout
this thesis project whenever it was required to represent the IL-10 structure
--- for visualization purposes, and for \textit{in silico} experiments.
...
The figure shows the IL-10 homodimer (which is the pre-dominant form of IL-10 in
...
domains, and each monomer contributes four helices to one domain, and two
helices to the other domain. The two domains are related by a two-fold
(\SI{180}{\degree}) rotational symmetry. From the point of view shown here, it
becomes obvious why IL-10 is sometimes described to have a \enquote{V-shape}, a
terminology also used in this thesis. }
...
(the monomers), each consisting of six helices, named A-F. The dimer has two
distinctive six-helix domains, and each monomer contributes four helices to one
domain, and two helices to the other domain: a domain is built by helices A-D of
one monomer together with helices E' and F' of the other monomer. For clarity,
this is color-encoded in \cref{fig:bg:il10_dimer_vshape}. The two monomers are
...
related. Considering the point of view shown in \cref{fig:bg:il10_dimer_vshape},
the overall shape of IL-10 is sometimes related to the letter V and therefore
called \enquote{V-shape}, which I will refer to later in this thesis.
...
two helices from a second IL-10 monomer, yielding a six-helix bundle domain. The
structure of each monomer is stabilized by two disulfide bridges which are shown
here in yellow.
}
