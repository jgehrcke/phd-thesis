...
We concluded that the results of a DMD study are rather insensitive with respect
to modest changes in the geometrical parameterization of DMD and that the
impact of the five N-terminal residues is not of the highest priority to study.


\subsubsection{Distinction among GAG types by single-run quantity cluster stats}

Ideally, the different behavior of different GAG types is expressed in the
...
should be discussed by means of an example. In one study, we used a heparin
hexasaccharide (dp6), in another we used a chondroitin-4-sulfate (CS4)
hexasaccharide. The heparin study yielded a single prominent cluster, the CS4
study yielded two similar clusters. The placement of the clusters relative to
IL-10 was similar (data not shown), and the quantitative evaluation of both
clusters yielded similar results, see
...
binding estimate are indistinguishable, considering the fluctuations represented
by the standard deviation. Just the cluster density indicates that the docking
solutions obtained for heparin show a little more spatial consistency in
themselves.
...
CS4 dp6 (second cluster) & 4               & 3.2                         & 2.4 $\pm$ 0.4          & -37 $\pm$ 14                          \\
\midrule
\end{tabular}
\caption{
...
study, we used a heparin hexasaccharide (dp6), in another we used a
...
estimate as provided by the MM-PBSA method. Standard deviations are derived
from the cluster-internal variations.}
