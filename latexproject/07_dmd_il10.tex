...
oriented from the bottom left to the upper right, with the GAG being placed in
the groove below R107. This is true for CS4 dp4, for both HA scenarios, and for
HP dp6. The other qualitatively different pose is oriented from the top left to
the bottom right, with the GAG placed above R107, and squeezed between R107 and
K119 (green an cyan in the figure, respectively). This is true for the HP dp4
scenario. The CS6 dp6 cluster is bent in a way that it can be considered a
combination of both principal binding poses.
...
on top of the DMD approach --- which increases the overall uncertainty --- it is
realistic to consider the SRED ensemble data as being more reliable than the
clustering result. Therefore, as of a lack of support from the SRED data, none
of both principal binding orientations discussed above must be excluded for
both, HP dp4 and dp6.


Looking at the structure ensembles shown in
\cref{fig:dmdil10:2nd_stage_all_clusters} it makes sense that K119 is identified
as contributing much to GAG binding: the cleft between R107 and K119 could serve
as a trap, potentially anchoring a GAG with one positively charged amino acid
residue from two sides simultaneously. The cleft is just wide enough for e.g.\
heparin, as can be seen in the HP dp4 panel of
\cref{fig:dmdil10:2nd_stage_all_clusters}.

 There is a difference
between dp4 and dp6 for heparin


HP dp4 \SI{2.2}{\angstrom}
HP dp6 \SI{2.6}{\angstrom}
HA dp4 \SI{3.0}{\angstrom}
HA dp6 \SI{2.6}{\angstrom}
CS4 dp6 \SI{3.4}{\angstrom}
CS6 dp6 \SI{2.7}{\angstrom}
