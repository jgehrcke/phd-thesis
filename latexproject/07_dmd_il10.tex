...
For the global design our our DMD studies, we decided to take an iterative
approach. In a first round, we aimed to gather experience about how the
IL-10-GAG system responds to various kinds of DMD condition changes.
Furthermore, since the technical conditions were unstable, we used this first
round of studies for detecting and resolving technical issues. Hence, in this
first round of studies, we varied the conditions according to all to all points
...
run identifiers. As a result, we obtained a table of dimensions $NxM$, with $N$
being the number of DMD runs and $M$ being the number of single-run-derivable
quantities. In a second class of analyses, the ensemble-derived quantities as
described in \cref{dmd:dataanalysis} were extracted from all runs in the DMD
study, usually based on the data extraction performed before. Finally, we
clustered the docking solution ensemble the method described in
\cref{chapter:bspred}.

These reduced set of data allows for a broad subsequent data analysis. For
instance, any two single-run-derivable quantities can easily be correlated, or
the distribution of any extracted quantity can easily be looked at. Furthermore,
given the \textit{clustering} of docking solutions, it makes sense to generate
cluster statistics, e.g.\ the average ligand mobility within a cluster. This
enables meaningful comparison of clusters among different DMD studies by those
properties.
