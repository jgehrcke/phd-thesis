...
focus the sampling onto that region: the new focus point was set onto the
central atom of a representative docking solution from above-mentioned clusters.
The C-alpha atom of M154 was kept as core atom, yielding a tMD target distance
of \SI{20.5}{\angstrom}.

Another optimization implemented in the second stage of DMD studies was the
reduction of the target displacement length for the LROM setup. It was shortened
from \SI{30}{\angstrom} to \SI{25}{\angstrom}, effectively decreasing the system
size and therefore the computational effort for performing the tMD simulations.
Furthermore, the pulling process simulation time was decreased from
\SI{4}{\nano\second} to \SI{3}{\nano\second}. Overall, 98 DMD runs were
completed for CS-4 dp6, 200 runs for CS-6 dp6, 198 runs for HA dp4, 278 runs for
HA dp6, 194 runs for HP dp4, and 298 runs for HP dp6.
...
Another observation is that for HP as well as HA, which have both been analyzed
...
(i.e.\ for HP, CS-4 and CS-6): R107, R106, R104, K119, T6, N10. This is another
...
introduced via the geometrical parameterization does not allow R102 to be
...
ligand, whereas normally it is not the terminus.
